var AnexosinfopymeC = UCPMC.extend({
	/**
	...
	init: function(){
		...
		console.log('%c'+sprintf(_("anexosinfopyme has been loaded at %s"),UCP.dateTimeFormatter(this.time)),'background: #222; color: #bada55');
		console.log('%c'+sprintf(_("anexosinfopyme has been loaded at %s"),UCP.timeFormatter(this.time)),'background: #222; color: #bada55');
		console.log('%c'+sprintf(_("anexosinfopyme has been loaded at %s"),UCP.dateFormatter(this.time)),'background: #222; color: #bada55');
	...
	prepoll: function() {
		var items = {};
		$(".grid-stack-item[data-rawname=anexosinfopyme][data-widget_type_id=poll]").each(function() {
			var id = $(this).data("id");
	...
	poll: function(data){
		...
		//$("#anexosinfopyme-badge").text(data.total);
		//$("#nav-btn-anexosinfopyme .badge").text(data.total);
	...
	connect: function() {
		...
		try {
			...
			UCP.wsconnect("anexosinfopyme", function(socket) {
				if (socket === false) {
					$this.socket = null;
					return false;
				} else {
